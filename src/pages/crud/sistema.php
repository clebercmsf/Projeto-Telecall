<?php
session_start();
include_once('config.php');
if (!isset($_SESSION['username']) || !isset($_SESSION['profile']) || $_SESSION['profile'] != "1") {
  session_unset();
  session_destroy();
  header('Location: sair.php');
  exit;
}

$logado = $_SESSION['username'];
if (!empty($_GET['search'])) {
  $data = $_GET['search'];
  $sql = "SELECT * FROM usuario WHERE id_Usuario LIKE '%$data%' or Nome_Usuario LIKE '%$data%' or Sexo_Usuario LIKE '%$data%' or CPF_Usuario LIKE '%$data%' 
  or Celular_Usuario LIKE '%$data%' or Telefone_Usuario LIKE '%$data%' or Endereco_Usuario LIKE '%$data%' or Login_Usuario LIKE '%$data%'
  or Senha_Usuario LIKE '%$data%' or Perfil_Usuario LIKE '%$data%' ORDER BY id_Usuario ASC";
} else {
  $sql = "SELECT * FROM usuario ORDER BY id_Usuario ASC";
}
$result = $conexao->query($sql);
?>
<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
  <link rel="stylesheet" href="./sistema/style/sistema.css" />
  <script src="./sistema/sistema.js" defer></script>
  <title>CRUD - Telecall</title>
  <link rel="icon" href="../../assets/img/telecall-logo4-light.png" />
</head>

<body>
<header>
    <nav class="nav-bar">
      <div class="logo">
        <a href="../home/home.php">
          <img src="../../assets/img/telecall-logo6-light.png" alt="telecall-logo" id="logo-header">
        </a>
      </div>
      <div class="buttons">
        <button onClick="window.print()" onChange="hidden" style="width: 100px;">Salvar PDF</button>
        <button onclick='logout();'>Sair</button>
      </div>
    </nav>
  </header>
  <br>
  <?php
  echo "<h1 class='noprint'>BEM VINDO <u>$logado</u></h1>";
  ?>
  <br>
  <div class="noprint">
    <input type="search" class="textfield" placeholder="Pesquisar" id="search">
    <button onclick="searchData()" class="btn btnP">
      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-search" viewBox="0 0 16 16">
        <path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z" />
      </svg>
    </button>
  </div>
  <div class="m-3">
    <table class="table text-white table-bg">
      <thead>
        <tr>
          <th scope="col">#</th>
          <th scope="col">Nome</th>
          <th scope="col">Data de Nascimento</th>
          <th scope="col">Sexo</th>
          <th scope="col">Nome Materno</th>
          <th scope="col">CPF</th>
          <th scope="col">Celular</th>
          <th scope="col">Telefone</th>
          <th scope="col">Endereço</th>
          <th scope="col">Login</th>
          <th scope="col">Senha</th>
          <th scope="col">Tipo de Perfil</th>
          <th scope="col">...</th>
        </tr>
      </thead>
      <tbody>
        <?php
        while ($user_data = mysqli_fetch_assoc($result)) {
          echo "<tr>";
          echo "<td>" . $user_data['id_Usuario'] . "</td>";
          echo "<td>" . $user_data['Nome_Usuario'] . "</td>";
          echo "<td>" . $user_data['Nascimento_Usuario'] . "</td>";
          echo "<td>" . $user_data['Sexo_Usuario'] . "</td>";
          echo "<td>" . $user_data['Nome_Materno'] . "</td>";
          echo "<td>" . $user_data['CPF_Usuario'] . "</td>";
          echo "<td>" . $user_data['Celular_Usuario'] . "</td>";
          echo "<td>" . $user_data['Telefone_Usuario'] . "</td>";
          echo "<td>" . $user_data['Endereco_Usuario'] . "</td>";
          echo "<td>" . $user_data['Login_Usuario'] . "</td>";
          echo "<td>" . $user_data['Senha_Usuario'] . "</td>";
          $profile = $user_data['Perfil_Usuario'];
          if ($profile == 1) {
            $profile = "Master";
          } elseif ($profile == 2) {
            $profile = "Comum";
          }
          echo "<td>" . $profile . "</td>";
          echo "<td>
                        <a class='btn btn-sm btn-primary' href='./edit/edit.php?id=$user_data[id_Usuario]' title='Editar'>
                            <svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='bi bi-pencil' viewBox='0 0 16 16'>
                                <path d='M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z'/>
                            </svg>
                            </a> 
                            <a class='btn btn-sm btn-danger' href='delete.php?id=$user_data[id_Usuario]' title='Deletar'>
                                <svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='bi bi-trash-fill' viewBox='0 0 16 16'>
                                    <path d='M2.5 1a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1H3v9a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V4h.5a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H10a1 1 0 0 0-1-1H7a1 1 0 0 0-1 1H2.5zm3 4a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 .5-.5zM8 5a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7A.5.5 0 0 1 8 5zm3 .5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 1 0z'/>
                                </svg>
                            </a>
                            </td>";
          echo "</tr>";
        }
        ?>
      </tbody>
    </table>
  </div>
</body>

</html>